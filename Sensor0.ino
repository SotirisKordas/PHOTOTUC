/*************************************************************************
   Sensor 0 unit - nRF24L01+ radio communications
 *                                                                       *
        This is the Sensor 0 unit of our system

        At initiliazitation stage the Sensor 0 system in TX mode waiting
        sending recognition messages to the Base system. 
        At User Config stage the Sensor 0 system stays in TX mode, 
        sending messages of sensor values.
        When User Config is done the Sensor 0 unit stays in TX mode, 
...
int sensorValue; //variable that holds the sensor value
...
// here we declare a few timers we are going to use
TimerObject *initRepeat = new TimerObject(960);
TimerObject *dataRepeat = new TimerObject(960);
TimerObject *threshRCRepeat = new TimerObject(460);

void setup() {
  // put your setup code here, to run once:
  randomSeed(analogRead(0)); //create unique seed value for random number generation
  initialDelay = random(100, 200);
...
void sendTrigger (int sensorValue) {
  ...
  if(sensorRadio.write(&sensorValue, sizeof(sensorValue))) {
    ...
    triggerState = SENSOR_TRIGG;
      ...
      //Serial.println("Trigger wasn't sent");
  }
}
