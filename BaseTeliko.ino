 ...
 *                                                                       *
        ...
        the Base stays in PRX mode, receiving messages of sensor values.
        When User Config is done the Base remains in PRX mode, 
        awaiting for the Sensors to send a possible trigger value.
...
// From -7 to -8. -7 means > and -8 means <
// For more sensors add corresponding values accordingly
int dummyOp[MAX_SENSORS] = {-7,-7}; //values smaller than the max number of sensors so that system doesn't get "confused" on Init and Config messages
...
// Variables that holds the number of sensors recognized in Initialization stage
// We need the opposite during user config stage
int sensorCtr = 0;
int negativeSensorCtr = 0;
...
int sensorRCsensorCtr [MAX_SENSORS] = {0,0};
...
TimerObject *triggTimer = new TimerObject(50);
byte triggCount = 0;
boolean triggTimeout = false; 

// These variables are timers. Depending on the delay the user chooses one of them will be activated if we have an AND function
TimerObject *delayTimer50 = new TimerObject(50);
TimerObject *delayTimer100 = new TimerObject(100);
TimerObject *delayTimer150 = new TimerObject(150);
TimerObject *delayTimer200 = new TimerObject(200);
TimerObject *delayTimer300 = new TimerObject(300);


byte delayCount = 0;
boolean delayTimeout = false; 
...
// Variable that stored the time the trigger delay started
unsigned long triggerDelayStart = 0;

// Variable that tells us if the trigger dealy is still running
...
// These variables are declared to help test the system delay
...
void setup() {
  ...
  delayTimer50 -> setOnTimer(&checkDelayTimeout);
  delayTimer100 -> setOnTimer(&checkDelayTimeout);
  delayTimer150 -> setOnTimer(&checkDelayTimeout);
  delayTimer200 -> setOnTimer(&checkDelayTimeout);
  delayTimer300 -> setOnTimer(&checkDelayTimeout);
  initStart -> Start();
      ...
      if(sensorCtr == 1) {
        ...
        if(newTrigger == true) {
          if(triggered == false) {
            ...
            while((newTrigger == false) && (triggered == false)){
              newTrigger = checkForTriggers();
            }
            if(triggered == false) {
              chooseTriggerDelay();
...
// This function is used to test the system delay
void checkTestPin (void) {
  if (hasTriggered == true) {
    digitalWrite(HIGH,PIN_TEST);
    digitalWrite(HIGH,PIN_FLASH);
  }
}
...
/******************************************************************************************
       When the user has chosen a dealy time between the trigger fucntion beeing true
       and actualy trigger of the flash/camera we have a timer that checks if this 
       delay time the user has chosen has elapsed. The timer uses this function
 ******************************************************************************************/
void checkDelayTimeout (void) {
  delayCount++;
  if(delayCount >= 1) {
    delayTimeout = true;
  }
  if (delayTimeout == true) {
...
void endInit (void) {
  if(sensorCtr != 0) {
    ...
    if(tmp == sensorCtr) {
      ...
      negativeSensorCtr = -sensorCtr;
      //Serial.println(negativeSensorCtr);
    } else {
...
//                 Serial.println(" got the operator");
  ...
  for (byte node = 0; node < MAX_SENSORS; node++) {
    if ((sensorRCThresh[node] == 1) && (sensorRCOP[node] == 1) && (sensorRCsensorCtr[node] != 1)) {
      baseRadio.stopListening();
      baseRadio.openWritingPipe(wAddresses[node]);
      if(baseRadio.write(&negativeSensorCtr, sizeof(negativeSensorCtr))) {
        //Serial.println("Actually sent the counter");
        //Serial.println(negativeSensorCtr);
        baseRadio.startListening(); 
        unsigned long startTimer = millis();
        bool timeout = false;
        ...
        if(timeout) {
          //Serial.println("Sensor did not respond for the sensor counter");
        } else {
            baseRadio.read(&gotByte, sizeof(gotByte));
            //Serial.println(gotByte);
            if(gotByte == negativeSensorCtr) {
              if(sensorRCsensorCtr[pipeNum - 1] == 0) {
                sensorRCsensorCtr[pipeNum - 1] = 1;
//                Serial.print("Sensor "); 
//                Serial.print(node); 
//                Serial.println(" got the counter");
                //if (sensorCtr > 1) {
                unsigned long startTimer = millis();
                while(millis() - startTimer < 2) {
...
boolean checkForTriggers (void) {
  ...
  if(baseRadio.available(&pipeNum)) {
    ...
    if((sensorRCInit[pipeNum - 1] == 1) && (triggerTrue[pipeNum - 1] == 0)) {
      if(sensorValue >= 0) {
        if(((dummyOp[pipeNum - 1] == -7) && (sensorValue > dummyZero[pipeNum - 1])) || ((dummyOp[pipeNum - 1] == -8) && (sensorValue < dummyZero[pipeNum - 1]))) {
 ...
 /*********************************************************************************
      This fucntion is used when we need to trigger the flash/camera
      so that we can have the delay time the user has chosen
 ********************************************************************************/
void chooseTriggerDelay (void) {
  if (triggerDelay == 50) {
    if (delayTimeout == false) {
      delayTimer50 -> Start();
      while (delayTimeout == false) {
        delayTimer50 -> Update();
      }
    } else if (delayTimeout == true) {
      delayTimer50 -> Stop();                               
    }
  } else if (triggerDelay == 100) {
      if (delayTimeout == false) {
        delayTimer100 -> Start();
        while (delayTimeout == false) {
          delayTimer100 -> Update();
        }
      } else if (delayTimeout == true) {
          delayTimer100 -> Stop();                               
      }
  } else if (triggerDelay == 150) {
      if (delayTimeout == false) {
        delayTimer150 -> Start();
        while (delayTimeout == false) {
          delayTimer150 -> Update();
        }
      } else if (delayTimeout == true) {
          delayTimer150 -> Stop();                               
      }
  } else if (triggerDelay == 200) {
      if (delayTimeout == false) {
        delayTimer200 -> Start();
        while (delayTimeout == false) {
          delayTimer200 -> Update();
        }
      } else if (delayTimeout == true) {
          delayTimer200 -> Stop();                               
      }
  } else if (triggerDelay == 300) {
      if (delayTimeout == false) {
        delayTimer300 -> Start();
        while (delayTimeout == false) {
          delayTimer300 -> Update();
        }
      } else if (delayTimeout == true) {
          delayTimer300 -> Stop();                               
      }
...
void drawConfigMenu (void) {
  ...
  display.setTextColor(WHITE);
  if (menuLayer == 1) {
    drawConfigMenuLayer1();
  } else if (menuLayer == 2) {
      drawConfigMenuLayer2();
  } else if (menuLayer == 3) {
      drawConfigMenuLayer3();
  } else if(menuLayer == 4) {
      drawConfigMenuLayer4();
  }
  screenRefresh(); 
}
...
//       display.println("got rec");
  ...
  if ((triggerCtr == sensorCtr) && (trigFail == false) && (triggerLogic == 0)) {
      display.println("System Triggered");
      //display.print(triggerDelay);
  } else if ((triggerCtr == sensorCtr) && (trigFail == true) && (triggerLogic == 0)) {
      display.print("System Trigger failed");
  } else if ((triggerCtr != 0) && (triggerLogic == 1)) {
      display.println("System Triggered");
      //display.print(triggerDelay);
  } else if (triggerCtr == 0) {
      display.print("No triggers yet");
  }
  screenRefresh();
}
 ...
 ***********************************************************************************/
 ...
 /***********************************************************************************
        The following lines of code are used to determine the action of a single
        button press on the third layer of the menu. To support more sensors 
        uncomment the comments from line 807 to 830 accordingly.
        Maximum of 6 sensors is suppoerted, maximum value of condition is 1700
 ***********************************************************************************/
  if (menuLayer == 3) {
...
void longPressStart (void) {
  ...
  if (menuLayer == 1) {
    ...
    ************************************************************************************************/
    ...
    /*************************************************************************************
          Following lines are used for long button press action when in "HELP" menu
    *************************************************************************************/  
    if (pic == 4100) {
      pic = 4110;
      menuLayer = menuLayer + 1;
...
/*************************************************************************
       ...
       To support more sensors uncommment the
       according choices. By default only two sensors are
       supported, so only choices 1100 and 1200 and their subsets are
...
void drawConfigMenuLayer2 () {
  drawConfigMenuLayer2Sensor();
  drawConfigMenuLayer2Delay();
  drawConfigMenuLayer2Function();
  drawConfigMenuLayer2Help();
  drawConfigMenuLayer2Confirm();
}

/**************************************************************************
     The following function configures what will be shown to the 
     menu screen when the user has selected to configure sensor settings. 
     Each choices is allocated to a specific sensors. By defalut only
     two sensors are supportes, so only choice 1100 and 1200 are shown.
     Uncomments the other choices according to the number of sensors.
***************************************************************************/
void drawConfigMenuLayer2Sensor () {
  //    LAYER 2  //
  //    CHOICE 1.1 //
  if (pic == 1100) {
    for (byte node = 0 ; node < MAX_SENSORS ; node++) {
...
/**************************************************************************
     The lines following this comment configure what will be shown to the 
     menu screen when the user has selected to configure a delay
     between the point in time when the trigger function becomes true
     and the point when the systems actually triggers
***************************************************************************/
void drawConfigMenuLayer2Delay () {
  //    LAYER 2  //
  //    CHOICE 2.1 //  
  if (pic == 2100) {
    if(currentPointLine <= 20) {
...
/**************************************************************************
     The following function configures what will be shown to the 
     menu screen when the user has selected to choose the operator
***************************************************************************/
void drawConfigMenuLayer2Function () {
  //    LAYER 2  //
  //    CHOICE 3.1 //  
  if (pic == 3100) {
...
/**************************************************************************
     The following function configures what will be shown to the 
     screen menu when user has chose to go the the "HELP" submenu
***************************************************************************/
void drawConfigMenuLayer2Help () {
  //    LAYER 2  //
  //    CHOICE 4.1 //  
  if (pic == 4100) {
    display.setCursor(0, currentPointLine);
...
/**************************************************************************
     The following function configures what will be shown to the 
     screen menu when the uses wants to confirm all the user choices and
     go into triggering mode
***************************************************************************/
void drawConfigMenuLayer2Confirm () {
  //    LAYER 2  //
  //    CHOICE 5.1 //  
  if (pic == 5100) {
    display.setCursor(0, currentPointLine);
...
void drawConfigMenuLayer3 () {
 /*******************************************************************************
         ...
         of the menu. This corresponds directly to the choice between
         a threshold or an operator. Also the other instance is to 
         display the project information in the "HELP" submenu.
         To support more sensors uncommment the according choices. 
         By default only two sensors are supported, so only the 
         sets of 1100 and 1200 are uncommented. Make sure to change 
         accordingly the conditions in button handling for correct navigation  
 *******************************************************************************/
  //    LAYER 3  //
  //    CHOICE 1.1.1 //
  if (pic == 1110) {
 ...
 */
  if (pic == 4110) {
    display.setCursor(0, currentPointLine); 
    display.print("PROJECT : PHOTOTUC");
    currentPointLine = currentPointLine + 8;
    display.setCursor(0, currentPointLine); 
    display.print("CREATOR : S. KORDAS");
    currentPointLine = currentPointLine + 8;
    display.setCursor(0, currentPointLine); 
...
/*************************************************************************
         The following functions is used for the fourth layer
         of the menu. It is devided in two child functions to be more 
         readeble. To support more sensors uncommment the
         according choices. By default only two sensors are
...
void drawConfigMenuLayer4() {
  drawConfigMenuLayer4Operator();
  drawConfigMenuLayer4Threshold();  
}
...
*******************************************************************************/
void drawConfigMenuLayer4Operator () {
  //    LAYER 4  //
  //    CHOICE 1.1.2.1 //
  if (pic == 1121) {
    display.setTextSize(2);
...
/*******************************************************************************
       This functions contains what the menu will show when choosing
       for the corresponding threshold of each sensor.
       Further below there are commented parts that can be unommented
       for support of more sensors. Uncomment accoring to the sensor number
*******************************************************************************/
void drawConfigMenuLayer4Threshold () {
  //    LAYER 4  //
  //    CHOICE 1.1.1.1 //
  if (pic == 1111) {
    display.setTextSize(2);
